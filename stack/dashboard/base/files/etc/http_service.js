  ...
  async runNotReadyServer(config) {
    ...
    this.notReadyServer.route({
      ...
      handler: (req, responseToolkit) => {
        this.log.debug(`Fortishield dashboard server is not ready yet ${req.method}:${req.url.href}.`); // If server is not ready yet, because plugins or core can perform
        ...
        return responseToolkit.response('Fortishield dashboard server is not ready yet').code(503).header('Retry-After', '30');
      }
    });
