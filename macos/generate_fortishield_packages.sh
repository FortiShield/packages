#!/bin/bash
set -x
# Program to build and package OSX fortishield-agent
...
FORTISHIELD_PATH="${SOURCES_DIRECTORY}/fortishield"
FORTISHIELD_SOURCE_REPOSITORY="https://github.com/fortishield/fortishield"
export CONFIG="${FORTISHIELD_PATH}/etc/preloaded-vars.conf"
...
function clean_and_exit() {
    ...
    rm "${CURRENT_PATH}"/specs/fortishield-agent.pkgproj-e
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Fortishield's installation. This also includes library files.
...
function build_package() {
    ...
    git clone --depth=1 -b ${BRANCH_TAG} ${FORTISHIELD_SOURCE_REPOSITORY} "${FORTISHIELD_PATH}"

    get_pkgproj_specs

    VERSION=$(cat ${FORTISHIELD_PATH}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe fortishield agent is already installed on this machine."
    ...
    ${CURRENT_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${FORTISHIELD_PATH}" ${JOBS}
    ...
    if packagesbuild ${AGENT_PKG_FILE} --build-folder ${DESTINATION} ; then
        echo "The fortishield agent package for macOS has been successfully built."
        pkg_name="fortishield-agent-${VERSION}-${REVISION}.${ARCH}.pkg"
        sign_pkg
...
function get_pkgproj_specs() {
    ...
    VERSION=$(< "${FORTISHIELD_PATH}/src/VERSION"  cut -d "-" -f1 | cut -c 2-)

    pkg_file="specs/fortishield-agent-${ARCH}.pkgproj"
...
function main() {
    ...
    if [[ "${BUILD}" != "no" ]]; then
        check_root
        AGENT_PKG_FILE="${CURRENT_PATH}/package_files/fortishield-agent-${ARCH}.pkgproj"
        build_package
        "${CURRENT_PATH}/uninstall.sh"
    fi
    if [ "${NOTARIZE}" = "yes" ]; then
