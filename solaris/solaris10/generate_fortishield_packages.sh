...
# Fortishield Solaris 10 Package builder.


# CONFIGURATION VARIABLES
fortishield_branch="$(echo "$2" | cut -d "/" -f2)"
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "fortishield-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "fortishield-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "fortishield-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "fortishield-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "fortishield-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "fortishield-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "fortishield-agent_$VERSION.proto"
    echo "f none /etc/init.d/fortishield-agent  0755 root root" >> "fortishield-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97fortishield-agent=/etc/init.d/fortishield-agent" >> "fortishield-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97fortishield-agent=/etc/init.d/fortishield-agent" >> "fortishield-agent_$VERSION.proto"
    cat "fortishield-agent_$VERSION.list" | pkgproto >> "fortishield-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "fortishield-agent_$VERSION.proto"
    pkg_name="fortishield-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" fortishield-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf fortishield-agent fortishield *.list *proto
    rm -f *.new
