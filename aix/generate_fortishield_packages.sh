...
build_package() {
  ...
  source_code="http://api.github.com/repos/fortishield/fortishield/tarball/${reference}"

  rm -f fortishield.tar.gz && curl -L ${source_code} -k -o fortishield.tar.gz -s
  rm -rf fortishield-fortishield-* fortishield-agent-*
  extracted_directory=$(gunzip -c fortishield.tar.gz | tar -xvf - | tail -n 1 | cut -d' ' -f2 | cut -d'/' -f1)
  fortishield_version=$(cat ${extracted_directory}/src/VERSION | cut -d'v' -f2)
  cp -pr ${extracted_directory} fortishield-agent-${fortishield_version}
  ...
  package_name=fortishield-agent-${fortishield_version}
  tar cf ${package_name}.tar ${package_name} && gzip ${package_name}.tar
  mv ${package_name}.tar.gz ${rpm_build_dir}/SOURCES/

  cp ${current_path}/SPECS/fortishield-agent-aix.spec ${rpm_build_dir}/SPECS/${package_name}-aix.spec
  ...
  find /etc/ -name "*fortishield*" -exec rm {} \;
