# Fortishield installer - manager.sh functions.
...
function manager_startCluster() {
    ...
    common_logger -d "Starting Fortishield manager cluster."
    ...
    key=$(tar -axf "${tar_file}" fortishield-install-files/clusterkey -O)
    ...
    eval 'sed -i -e "${lstart},${lend}s/<name>.*<\/name>/<name>fortishield_cluster<\/name>/" \
...
function manager_configure(){
    ...
    common_logger -d "Configuring Fortishield manager."
    ...
    common_logger -d "Setting provisional Fortishield indexer password."
    eval "/var/ossec/bin/fortishield-keystore -f indexer -k username -v admin"
    eval "/var/ossec/bin/fortishield-keystore -f indexer -k password -v admin"  
    common_logger "Fortishield manager vulnerability detection configuration finished."
}

function manager_install() {

    common_logger "Starting the Fortishield manager installation."
    if [ "${sys_type}" == "yum" ]; then
        installCommon_yumInstall "fortishield-manager" "${fortishield_version}-*"
    elif [ "${sys_type}" == "apt-get" ]; then
        installCommon_aptInstall "fortishield-manager" "${fortishield_version}-*"
    fi

    common_checkInstalled
    if [  "$install_result" != 0  ] || [ -z "${fortishield_installed}" ]; then
        common_logger -e "Fortishield installation failed."
        installCommon_rollBack
        exit 1
    else
        common_logger "Fortishield manager installation finished."
    fi
}
