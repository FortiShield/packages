...
# Fortishield installer: offline download
...
function offline_download() {
  ...
  common_logger "Starting Fortishield packages download."
  common_logger "Downloading Fortishield ${package_type} packages for ${arch}."
  dest_path="${base_dest_folder}/fortishield-packages"
  ...
  if [ "${package_type}" == "rpm" ]; then
    manager_rpm_package="fortishield-manager-${fortishield_version}-${manager_revision}.x86_64.${package_type}"
    indexer_rpm_package="fortishield-indexer-${fortishield_version}-${indexer_revision}.x86_64.${package_type}"
    dashboard_rpm_package="fortishield-dashboard-${fortishield_version}-${dashboard_revision}.x86_64.${package_type}"
    ...
    manager_deb_package="fortishield-manager_${fortishield_version}-${manager_revision}_amd64.${package_type}"
    indexer_deb_package="fortishield-indexer_${fortishield_version}-${indexer_revision}_amd64.${package_type}"
    dashboard_deb_package="fortishield-dashboard_${fortishield_version}-${dashboard_revision}_amd64.${package_type}"
  ...
  while common_curl -s -I -o /dev/null -w "%{http_code}" "${manager_base_url}/${manager_package}" --max-time 300 --retry 5 --retry-delay 5 --fail | grep -q "200"; do
    ...
    if [ "${package_type}" == "rpm" ]; then
      manager_rpm_package="fortishield-manager-${fortishield_version}-${manager_revision}.x86_64.rpm"
      manager_package="${manager_rpm_package}"
    else
      manager_deb_package="fortishield-manager_${fortishield_version}-${manager_revision}_amd64.deb"
      manager_package="${manager_deb_package}"
    fi
  done
  ...
  common_logger -d "Fortishield manager package revision fetched."
  ...
  while common_curl -s -I -o /dev/null -w "%{http_code}" "${indexer_base_url}/${indexer_package}" --max-time 300 --retry 5 --retry-delay 5 --fail | grep -q "200"; do
    ...
    if [ "${package_type}" == "rpm" ]; then
      indexer_rpm_package="fortishield-indexer-${fortishield_version}-${indexer_revision}.x86_64.rpm"
      indexer_package="${indexer_rpm_package}"
    else
      indexer_deb_package="fortishield-indexer_${fortishield_version}-${indexer_revision}_amd64.deb"
      indexer_package="${indexer_deb_package}"
    fi
  done
  ...
  common_logger -d "Fortishield indexer package revision fetched."
  ...
  while common_curl -s -I -o /dev/null -w "%{http_code}" "${dashboard_base_url}/${dashboard_package}" --max-time 300 --retry 5 --retry-delay 5 --fail | grep -q "200"; do
    ...
    if [ "${package_type}" == "rpm" ]; then
      dashboard_rpm_package="fortishield-dashboard-${fortishield_version}-${dashboard_revision}.x86_64.rpm"
      dashboard_package="${dashboard_rpm_package}"
    else
      dashboard_deb_package="fortishield-dashboard_${fortishield_version}-${dashboard_revision}_amd64.deb"
      dashboard_package="${dashboard_deb_package}"
    fi
  done
  ...
  common_logger -d "Fortishield dashboard package revision fetched."

  for package in "${packages_to_download[@]}"
  do
    common_logger -d "Downloading Fortishield ${package} package..."
...
# --------------------------------------------------
  ...
  dest_path="${base_dest_folder}/fortishield-files"
  ...
  files_to_download=( "${fortishield_gpg_key}" "${filebeat_config_file}" "${filebeat_fortishield_template}" "${filebeat_fortishield_module}" )
  ...
  common_logger "The configuration files and assets are in fortishield-offline.tar.gz"
  ...
  common_logger "You can follow the installation guide here https://documentation.fortishield.github.io/current/deployment-options/offline-installation.html"
