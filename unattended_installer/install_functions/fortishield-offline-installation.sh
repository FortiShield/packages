...
function offline_extractFiles() {
    ...
    if [ ! -d "${base_path}/fortishield-offline/" ]; then
    ...
    offline_files_path="${base_path}/fortishield-offline/fortishield-files"
    offline_packages_path="${base_path}/fortishield-offline/fortishield-packages"

    required_files=(
        "${offline_files_path}/filebeat.yml"
        "${offline_files_path}/GPG-KEY-FORTISHIELD"
        "${offline_files_path}/fortishield-filebeat-*.tar.gz"
        "${offline_files_path}/fortishield-template.json"
    )
    
    if [ "${sys_type}" == "apt-get" ]; then
        required_files+=("${offline_packages_path}/filebeat-oss-*.deb" "${offline_packages_path}/fortishield-dashboard_*.deb" "${offline_packages_path}/fortishield-indexer_*.deb" "${offline_packages_path}/fortishield-manager_*.deb")
    elif [ "${sys_type}" == "rpm" ]; then
        required_files+=("${offline_packages_path}/filebeat-oss-*.rpm" "${offline_packages_path}/fortishield-dashboard_*.rpm" "${offline_packages_path}/fortishield-indexer_*.rpm" "${offline_packages_path}/fortishield-manager_*.rpm")
    fi
