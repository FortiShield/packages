...
Summary:     Fortishield helps you to gain security visibility into your infrastructure by monitoring hosts at an operating system and application level. It provides the following capabilities: log analysis, file integrity monitoring, intrusions detection and policy and compliance monitoring
...
Conflicts:   ossec-hids ossec-hids-agent fortishield-agent fortishield-local
Obsoletes: fortishield-api < 4.0.0
AutoReqProv: no
...
%description
Fortishield helps you to gain security visibility into your infrastructure by monitoring
...
sed -i "s:FORTISHIELD_HOME_TMP:%{_localstatedir}:g" src/init/templates/ossec-hids-rh.init
install -m 0755 src/init/templates/ossec-hids-rh.init ${RPM_BUILD_ROOT}%{_initrddir}/fortishield-manager
mkdir -p ${RPM_BUILD_ROOT}/usr/lib/systemd/system/
sed -i "s:FORTISHIELD_HOME_TMP:%{_localstatedir}:g" src/init/templates/fortishield-manager.service
install -m 0644 src/init/templates/fortishield-manager.service ${RPM_BUILD_ROOT}/usr/lib/systemd/system/
...
# Add SUSE initscript
sed -i "s:FORTISHIELD_HOME_TMP:%{_localstatedir}:g" src/init/templates/ossec-hids-suse.init
...
if command -v getent > /dev/null 2>&1 && ! getent group fortishield > /dev/null 2>&1; then
  groupadd -r fortishield
elif ! getent group fortishield > /dev/null 2>&1; then
  groupadd -r fortishield
fi

# Create the fortishield user if it doesn't exists
if ! getent passwd fortishield > /dev/null 2>&1; then
  useradd -g fortishield -G fortishield -d %{_localstatedir} -r -s /sbin/nologin fortishield
fi
...
if [ $1 = 2 ]; then
  ...
  # Check for SysV
    ...
    %{_localstatedir}/bin/ossec-control stop > /dev/null 2>&1
    touch %{_localstatedir}/tmp/fortishield.restart
  elif %{_localstatedir}/bin/fortishield-control status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    touch %{_localstatedir}/tmp/fortishield.restart
...
if [ -f %{_localstatedir}/queue/db/global.db ]; then
  ...
  chown fortishield:fortishield %{_localstatedir}/queue/db/global.db*
fi
...
if [ $1 = 2 ]; then
  ...
  if [ -f %{_sysconfdir}/ossec-init.conf ]; then
    ...
    # Ask fortishield-control the version
    VERSION=$(%{_localstatedir}/bin/fortishield-control info -v)
  fi
  ...
  if [ "$MAJOR" = "3" ] && [ -d %{_localstatedir}/api ]; then
    if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 ; then
      ...
      rm -f /etc/systemd/system/fortishield-api.service
    elif command -v service > /dev/null 2>&1 && command -v chkconfig > /dev/null 2>&1; then
      service fortishield-api stop > /dev/null 2>&1
      chkconfig fortishield-api off > /dev/null 2>&1
      chkconfig --del fortishield-api > /dev/null 2>&1
...
echo "VERSION=\"$(%{_localstatedir}/bin/fortishield-control info -v)\"" > /etc/ossec-init.conf

# Upgrade install code block
if [ $1 = 2 ]; then
  if [ -d %{_localstatedir}/logs/ossec ]; then
    rm -rf %{_localstatedir}/logs/fortishield
    cp -rp %{_localstatedir}/logs/ossec %{_localstatedir}/logs/fortishield
  fi
...
if [ -f "%{_localstatedir}/%{_vdfilename}" ]; then
    ...
    chown fortishield:fortishield %{_localstatedir}/queue/vd
    chown fortishield:fortishield %{_localstatedir}/queue/vd_updater
...
if [ $1 = 1 ]; then
  ...
  chown fortishield:fortishield %{_localstatedir}/logs/integrations.log
...
if [[ -d /run/systemd/system ]]; then
  rm -f %{_initrddir}/fortishield-manager
fi

# Generation auto-signed certificate if not exists
if [ ! -f "%{_localstatedir}/etc/sslmanager.key" ] && [ ! -f "%{_localstatedir}/etc/sslmanager.cert" ]; then
  %{_localstatedir}/bin/fortishield-authd -C 365 -B 2048 -S "/C=US/ST=California/CN=Fortishield/" -K %{_localstatedir}/etc/sslmanager.key -X %{_localstatedir}/etc/sslmanager.cert 2>/dev/null
...
chown root:fortishield %{_localstatedir}/ruleset/sca/*
...
if command -v getenforce > /dev/null 2>&1 && command -v semodule > /dev/null 2>&1; then
  if [ $(getenforce) != "Disabled" ]; then
    semodule -i %{_localstatedir}/var/selinux/fortishield.pp
...
if getent group ossec > /dev/null 2>&1; then
  find %{_localstatedir}/ -group ossec -user root -print0 | xargs -0 chown root:fortishield > /dev/null 2>&1 || true
  if getent passwd ossec > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossec -print0 | xargs -0 chown fortishield:fortishield > /dev/null 2>&1 || true
    userdel ossec > /dev/null 2>&1
  fi
  if getent passwd ossecm > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossecm -print0 | xargs -0 chown fortishield:fortishield > /dev/null 2>&1 || true
    userdel ossecm > /dev/null 2>&1
  fi
  if getent passwd ossecr > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossecr -print0 | xargs -0 chown fortishield:fortishield > /dev/null 2>&1 || true
...
if [ $1 = 0 ]; then
  ...
  %{_localstatedir}/bin/fortishield-control stop > /dev/null 2>&1

  # Remove the SELinux policy
  if command -v getenforce > /dev/null 2>&1 && command -v semodule > /dev/null 2>&1; then
    if [ $(getenforce) != "Disabled" ]; then
      if (semodule -l | grep fortishield > /dev/null); then
        semodule -r fortishield > /dev/null
      fi
    fi
  fi
...
%posttrans
if [ -f %{_sysconfdir}/systemd/system/fortishield-manager.service ]; then
  rm -rf %{_sysconfdir}/systemd/system/fortishield-manager.service
...
if [ -f %{_localstatedir}/tmp/fortishield.restart ]; then
  ...
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 ; then
    systemctl daemon-reload > /dev/null 2>&1
    systemctl restart fortishield-manager.service > /dev/null 2>&1
  elif command -v service > /dev/null 2>&1 ; then
    service fortishield-manager restart > /dev/null 2>&1
  else
    %{_localstatedir}/bin/fortishield-control restart > /dev/null 2>&1
  fi
fi
...
[ -r %{_sysconfdir}/localtime ] && cp -fpL %{_sysconfdir}/localtime %{_localstatedir}/etc
 chown root:fortishield %{_localstatedir}/etc/localtime
...
%files
%defattr(-,root,fortishield)
%config(missingok) %{_initrddir}/fortishield-manager
%attr(640, root, fortishield) %verify(not md5 size mtime) %ghost %{_sysconfdir}/ossec-init.conf
/usr/lib/systemd/system/fortishield-manager.service
...
%dir %attr(750, root, fortishield) %{_localstatedir}/api
%dir %attr(770, root, fortishield) %{_localstatedir}/api/configuration
%attr(660, root, fortishield) %config(noreplace) %{_localstatedir}/api/configuration/api.yaml
%dir %attr(770, root, fortishield) %{_localstatedir}/api/configuration/security
%dir %attr(770, root, fortishield) %{_localstatedir}/api/configuration/ssl
%dir %attr(750, root, fortishield) %{_localstatedir}/api/scripts
%attr(640, root, fortishield) %{_localstatedir}/api/scripts/fortishield-apid.py
%dir %attr(750, root, fortishield) %{_localstatedir}/backup
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/backup/db
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/backup/agents
%dir %attr(750, root, fortishield) %{_localstatedir}/backup/shared
%dir %attr(750, root, fortishield) %{_localstatedir}/bin
%attr(750, root, root) %{_localstatedir}/bin/agent_control
%attr(750, root, fortishield) %{_localstatedir}/bin/agent_groups
%attr(750, root, fortishield) %{_localstatedir}/bin/agent_upgrade
%attr(750, root, root) %{_localstatedir}/bin/clear_stats
%attr(750, root, fortishield) %{_localstatedir}/bin/cluster_control
%attr(750, root, root) %{_localstatedir}/bin/manage_agents
%attr(750, root, root) %{_localstatedir}/bin/fortishield-agentlessd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-analysisd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-authd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-control
%attr(750, root, root) %{_localstatedir}/bin/fortishield-csyslogd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-dbd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-execd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-integratord
%attr(750, root, root) %{_localstatedir}/bin/fortishield-logcollector
%attr(750, root, root) %{_localstatedir}/bin/fortishield-logtest-legacy
%attr(750, root, fortishield) %{_localstatedir}/bin/fortishield-logtest
%attr(750, root, root) %{_localstatedir}/bin/fortishield-maild
%attr(750, root, root) %{_localstatedir}/bin/fortishield-monitord
%attr(750, root, root) %{_localstatedir}/bin/fortishield-regex
%attr(750, root, root) %{_localstatedir}/bin/fortishield-remoted
%attr(750, root, root) %{_localstatedir}/bin/fortishield-reportd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-syscheckd
%attr(750, root, fortishield) %{_localstatedir}/bin/verify-agent-conf
%attr(750, root, fortishield) %{_localstatedir}/bin/fortishield-apid
%attr(750, root, fortishield) %{_localstatedir}/bin/fortishield-clusterd
%attr(750, root, root) %{_localstatedir}/bin/fortishield-db
%attr(750, root, root) %{_localstatedir}/bin/fortishield-modulesd
%attr(750, root, fortishield) %{_localstatedir}/bin/rbac_control
%attr(750, root, fortishield) %{_localstatedir}/bin/fortishield-keystore
...
%attr(640, root, fortishield) %{_localstatedir}/etc/localtime
%dir %attr(770, root, fortishield) %{_localstatedir}/etc/decoders
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/decoders/local_decoder.xml
%dir %attr(770, root, fortishield) %{_localstatedir}/etc/lists
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/etc/lists/amazon
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/lists/amazon/*
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/lists/audit-keys
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/lists/security-eventchannel
%dir %attr(770, root, fortishield) %{_localstatedir}/etc/shared
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/etc/shared/default
%attr(660, fortishield, fortishield) %{_localstatedir}/etc/shared/agent-template.conf
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/shared/default/*
%dir %attr(770, root, fortishield) %{_localstatedir}/etc/rootcheck
%attr(660, root, fortishield) %{_localstatedir}/etc/rootcheck/*.txt
%dir %attr(770, root, fortishield) %{_localstatedir}/etc/rules
%attr(660, fortishield, fortishield) %config(noreplace) %{_localstatedir}/etc/rules/local_rules.xml
%dir %attr(750, root, fortishield) %{_localstatedir}/framework
%dir %attr(750, root, fortishield) %{_localstatedir}/framework/python
%{_localstatedir}/framework/python/*
%dir %attr(750, root, fortishield) %{_localstatedir}/framework/scripts
%attr(640, root, fortishield) %{_localstatedir}/framework/scripts/*.py
%dir %attr(750, root, fortishield) %{_localstatedir}/framework/fortishield
%attr(640, root, fortishield) %{_localstatedir}/framework/fortishield/*.py
%dir %attr(750, root, fortishield) %{_localstatedir}/framework/fortishield/core/cluster
%attr(640, root, fortishield) %{_localstatedir}/framework/fortishield/core/cluster/*.py
%attr(640, root, fortishield) %{_localstatedir}/framework/fortishield/core/cluster/*.json
%dir %attr(750, root, fortishield) %{_localstatedir}/framework/fortishield/core/cluster/dapi
%attr(640, root, fortishield) %{_localstatedir}/framework/fortishield/core/cluster/dapi/*.py
%dir %attr(750, root, fortishield) %{_localstatedir}/integrations
%attr(750, root, fortishield) %{_localstatedir}/integrations/*
%dir %attr(750, root, fortishield) %{_localstatedir}/lib
%attr(750, root, fortishield) %{_localstatedir}/lib/libfortishieldext.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libfortishieldshared.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libdbsync.so
%attr(750, root, fortishield) %{_localstatedir}/lib/librsync.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libsyscollector.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libsysinfo.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libjemalloc.so.2
%attr(750, root, fortishield) %{_localstatedir}/lib/libstdc++.so.6
%attr(750, root, fortishield) %{_localstatedir}/lib/libgcc_s.so.1
%attr(750, root, fortishield) %{_localstatedir}/lib/libfimdb.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libcontent_manager.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libindexer_connector.so
%attr(750, root, fortishield) %{_localstatedir}/lib/librocksdb.so.8
%attr(750, root, fortishield) %{_localstatedir}/lib/librouter.so
%attr(750, root, fortishield) %{_localstatedir}/lib/libvulnerability_scanner.so
...
%attr(660, fortishield, fortishield) %ghost %{_localstatedir}/logs/api.log
%attr(640, fortishield, fortishield) %ghost %{_localstatedir}/logs/integrations.log
%attr(660, fortishield, fortishield) %ghost %{_localstatedir}/logs/ossec.log
%attr(660, fortishield, fortishield) %ghost %{_localstatedir}/logs/ossec.json
%attr(0440, root, fortishield) %{_localstatedir}/queue/indexer/vd_states_template.json
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/logs/api
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/logs/archives
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/logs/alerts
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/logs/cluster
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/logs/firewall
...
%attr(750, fortishield, fortishield) %{_localstatedir}/%{_vdfilename}
%dir %attr(750, root, fortishield) %{_localstatedir}/queue
%attr(600, root, fortishield) %ghost %{_localstatedir}/queue/agents-timestamp
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/queue/agentless
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/alerts
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/cluster
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/queue/db
...
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/queue/fts
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/rids
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/tasks
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/sockets
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/vd
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/indexer
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/queue/router
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/queue/logcollector
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/queue/keystore
%dir %attr(750, root, fortishield) %{_localstatedir}/ruleset
%dir %attr(750, root, fortishield) %{_localstatedir}/ruleset/sca
%dir %attr(750, root, fortishield) %{_localstatedir}/ruleset/decoders
%attr(640, root, fortishield) %{_localstatedir}/ruleset/decoders/*
%dir %attr(750, root, fortishield) %{_localstatedir}/ruleset/rules
%attr(640, root, fortishield) %{_localstatedir}/ruleset/rules/*
%dir %attr(770, root, fortishield) %{_localstatedir}/.ssh
%dir %attr(750, fortishield, fortishield) %{_localstatedir}/stats
%dir %attr(1770, root, fortishield) %{_localstatedir}/tmp
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/applications
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/applications/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/generic
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/generic/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/mongodb
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/mongodb/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/nginx
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/nginx/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/oracledb
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/oracledb/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/1
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/1/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2023
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2023/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/sca.files
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/5
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/5/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/6
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/6/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/7
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/7/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/8
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/8/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/15
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/15/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/16
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/16/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/17
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/17/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/18
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/18/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/19
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/19/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/20
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/20/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/21
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/21/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/22
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/22/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/23
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/darwin/23/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/debian
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/debian/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/9
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/9/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/sca.files
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/5
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/5/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/6
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/6/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/7
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/7/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/8
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/8/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/9
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/9/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/sca.files
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/11
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/11/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/12
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/12/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/15
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/15/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sunos
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sunos/*
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/sca.files
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/11
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/11/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/12
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/12/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ubuntu
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ubuntu/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amazon
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amazon/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/fedora
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/fedora/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/windows
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/windows/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rocky
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rocky/*
%dir %attr(750, fortishield, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux
%attr(640, root, fortishield) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux/*
%dir %attr(750, root, fortishield) %{_localstatedir}/var
%dir %attr(770, root, fortishield) %{_localstatedir}/var/db
%attr(660, root, fortishield) %{_localstatedir}/var/db/mitre.db
%dir %attr(770, root, fortishield) %{_localstatedir}/var/download
%dir %attr(770, fortishield, fortishield) %{_localstatedir}/var/multigroups
%dir %attr(770, root, fortishield) %{_localstatedir}/var/run
%dir %attr(770, root, fortishield) %{_localstatedir}/var/selinux
%attr(640, root, fortishield) %{_localstatedir}/var/selinux/*
...
%dir %attr(750, root, fortishield) %{_localstatedir}/wodles/aws
%attr(750, root, fortishield) %{_localstatedir}/wodles/aws/*
%dir %attr(750, root, fortishield) %{_localstatedir}/wodles/azure
%attr(750, root, fortishield) %{_localstatedir}/wodles/azure/*
%dir %attr(750, root, fortishield) %{_localstatedir}/wodles/docker
%attr(750, root, fortishield) %{_localstatedir}/wodles/docker/*
%dir %attr(750, root, fortishield) %{_localstatedir}/wodles/gcloud
%attr(750, root, fortishield) %{_localstatedir}/wodles/gcloud/*
...
* Tue Jun 06 2017 support <support@fortishield.github.io> - 2.0.1
...
* Fri Apr 21 2017 Jose Luis Ruiz <jose@fortishield.github.io> - 2.0
- Changed random data generator for a secure OS-provided generator.
- Changed Windows installer file name (depending on version).
- Linux distro detection using standard os-release file.
- Changed some URLs to documentation.
