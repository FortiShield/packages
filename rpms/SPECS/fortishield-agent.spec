...
%else
    ...
    make deps RESOURCES_URL=http://fortishield.github.io/packages/deps/${deps_version} TARGET=agent
...
install -m 0755 src/init/templates/ossec-hids-rh.init ${RPM_BUILD_ROOT}%{_initrddir}/fortishield-agent
sed -i "s:FORTISHIELD_HOME_TMP:%{_localstatedir}:g" src/init/templates/fortishield-agent.service
install -m 0644 src/init/templates/fortishield-agent.service ${RPM_BUILD_ROOT}/usr/lib/systemd/system/
...
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet fortishield-agent > /dev/null 2>&1; then
    systemctl stop fortishield-agent.service > /dev/null 2>&1
    touch %{_localstatedir}/tmp/fortishield.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service fortishield-agent status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service fortishield-agent stop > /dev/null 2>&1
    touch %{_localstatedir}/tmp/fortishield.restart
...
if [ $1 = 1 ]; then
  ...
  # Register and configure agent if Fortishield environment variables are defined
...
if [[ -d /run/systemd/system ]]; then
  rm -f %{_initrddir}/fortishield-agent
fi
...
if ([ "X${DIST_NAME}" = "Xrhel" ] || [ "X${DIST_NAME}" = "Xcentos" ] || [ "X${DIST_NAME}" = "XCentOS" ]) && [ "${DIST_VER}" == "5" ]; then
  if command -v getenforce > /dev/null 2>&1; then
    if [ $(getenforce) !=  "Disabled" ]; then
      chcon -t textrel_shlib_t  %{_localstatedir}/lib/libfortishieldext.so
      chcon -t textrel_shlib_t  %{_localstatedir}/lib/libfortishieldshared.so
    fi
  fi
else
...
%posttrans
if [ -f %{_sysconfdir}/systemd/system/fortishield-agent.service ]; then
  rm -rf %{_sysconfdir}/systemd/system/fortishield-agent.service
...
if [ -f %{_localstatedir}/tmp/fortishield.restart ]; then
  ...
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 ; then
    systemctl daemon-reload > /dev/null 2>&1
    systemctl restart fortishield-agent.service > /dev/null 2>&1
  elif command -v service > /dev/null 2>&1; then
    service fortishield-agent restart > /dev/null 2>&1
  else
...
%files
%defattr(-,root,root)
%config(missingok) %{_initrddir}/fortishield-agent
%attr(640, root, fortishield) %verify(not md5 size mtime) %ghost %{_sysconfdir}/ossec-init.conf
/usr/lib/systemd/system/fortishield-agent.service
...
- Fixed daemon list for service reloading at fortishield-control.
