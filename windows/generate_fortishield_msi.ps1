    ...
    USAGE:
        ...
        * FORTISHIELD:
          $ ./generate_fortishield_msi.ps1  -OPTIONAL_REVISION {{ REVISION }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            ...
            Build a devel msi:    $ ./generate_fortishield_msi.ps1 -OPTIONAL_REVISION 2 -SIGN no
            Build a prod msi:     $ ./generate_fortishield_msi.ps1 -OPTIONAL_REVISION 1 -SIGN yes -
    "
    Exit
}
...
function ComputeMsiName() {
    ...
    $MSI_NAME="fortishield-agent-$VERSION-$OPTIONAL_REVISION.msi"
    return $MSI_NAME
}

function BuildFortishieldMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\fortishield_modules\syscollector\build\bin\syscollector.dll"
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\syscheckd\build\bin\libfimdb.dll"
    }

    Write-Host "Building MSI installer..."

    & $CANDLE_EXE -nologo .\fortishield-installer.wxs -out "fortishield-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\fortishield-installer.wixobj" -out $MSI_NAME  -ext WixUtilExtension -ext WixUiExtension
