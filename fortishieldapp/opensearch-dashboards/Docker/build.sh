...
packages_list=( "main_fortishield" "fortishield-check-updates_fortishieldCheckUpdates" "fortishield-core_fortishieldCore" )
...
fortishield_app_nvmrc_url="${fortishield_app_raw_repo_url}/${fortishield_branch}/.nvmrc"
...
prepare_env() {
    echo "Downloading package.json and .nvmrc from fortishield-dashboard-plugins repository"
    if ! curl $fortishield_app_package_json_url -o "/tmp/package.json" ; then
        echo "Error downloading package.json from GitHub."
        exit 1
    fi

    if ! curl $fortishield_app_nvmrc_url -o "/tmp/.nvmrc" ; then
...
download_fortishield_app_sources() {
    if ! git clone $fortishield_app_clone_repo_url --branch ${fortishield_branch} --depth=1 ${git_clone_tmp_dir}; then
        echo "Error downloading the source code from fortishield-dashboard-app GitHub repository."
        exit 1
    fi
...
build_package(){
    ...
    for item in ${packages_list[@]}; do
        ...
        if [ -z "${app_revision}" ]; then
            fortishield_app_pkg_name="${array[1]}-${fortishield_version}.zip"
        else
            fortishield_app_pkg_name="${array[1]}-${fortishield_version}-${app_revision}.zip"
        fi
        ...
        find "${source_dir}/${array[0]}" -name "*.zip" -exec mv {} ${destination_dir}/${fortishield_app_pkg_name} \;
