...
@setup-test {
    ...
    k_certs_path="/etc/fortishield-dashboard/certs/"
...
function load-dashboard_copyCertificates() {
    ...
    dashboard_cert_path="/etc/fortishield-dashboard/certs/"
}
...
test-02-dashboard_copyCertificates-assert() {
    rm -f /etc/fortishield-dashboard/certs/*
    tar -xf /tmp/tarfile.tar -C /etc/fortishield-dashboard/certs/ ./dashboard1.pem  && mv /etc/fortishield-dashboard/certs/dashboard1.pem /etc/fortishield-dashboard/certs/dashboard.pem
    tar -xf /tmp/tarfile.tar -C /etc/fortishield-dashboard/certs/ ./dashboard1-key.pem  && mv /etc/fortishield-dashboard/certs/dashboard1-key.pem /etc/fortishield-dashboard/certs/dashboard-key.pem
    tar -xf /tmp/tarfile.tar -C /etc/fortishield-dashboard/certs/ ./root-ca.pem
    chown -R fortishield-dashboard:fortishield-dashboard /etc/fortishield-dashboard/
    chmod -R 500 /etc/fortishield-dashboard/certs/
    chmod 440 /etc/fortishield-dashboard/certs/*
}
...
test-03-dashboard_install-yum-assert() {
    yum install fortishield-dashboard-1.13.2-1 -y
}
...
test-ASSERT-FAIL-04-dashboard_install-yum-error() {
    ...
    fortishield_version="5.0.0"
    fortishield_revision="1"
    @mockfalse yum install fortishield-dashboard-1.13.2-1 -y
    dashboard_install
}
...
test-05-dashboard_install-apt-assert() {
    apt install fortishield-dashboard=1.13.2-1 -y
}
...
test-ASSERT-FAIL-06-dashboard_install-apt-error() {
    ...
    fortishield_version="5.0.0"
    fortishield_revision="1"
    @mockfalse apt install fortishield-dashboard=1.13.2-1 -y
    dashboard_install
}
...
test-07-dashboard_configure-dist-one-kibana-node-one-elastic-node-assert() {
    dashboard_copyCertificates
    installCommon_getConfig dashboard/dashboard_unattended_distributed.yml /etc/fortishield-dashboard/opensearch_dashboards.yml
}
...
test-09-dashboard_configure-AIO-assert() {
    dashboard_copyCertificates
    installCommon_getConfig dashboard/dashboard_unattended.yml /etc/fortishield-dashboard/opensearch_dashboards.yml
}
...
test-10-dashboard_initialize-distributed-one-kibana-node-one-fortishield-node-curl-correct() {
    ...
    server_node_names=("fortishield1")
    server_node_ips=("2.2.2.2")
    dashboard_initialize
}

test-10-dashboard_initialize-distributed-one-kibana-node-one-fortishield-node-curl-correct-assert() {
    installCommon_getPass "admin"
    sed -i 's,url: https://localhost,url: https://2.2.2.2,g' /usr/share/fortishield-dashboard/data/fortishield/config/fortishield.yml
}

test-ASSERT-FAIL-11-dashboard_initialize-distributed-one-kibana-node-one-fortishield-node-curl-error() {
    load-dashboard_initialize
    dashboard_node_names=("kibana1")
    dashboard_node_ips=("1.1.1.1")
    u_pass="user_password"
...
test-12-dashboard_initialize-distributed-two-kibana-nodes-two-fortishield-nodes-curl-correct() {
    load-dashboard_initialize
    dashboard_node_names=("kibana1" "kibana2")
...
test-12-dashboard_initialize-distributed-two-kibana-nodes-two-fortishield-nodes-curl-correct-assert() {
    installCommon_getPass "admin"
    sed -i 's,url: https://localhost,url: https://1.1.2.2,g' /usr/share/fortishield-dashboard/data/fortishield/config/fortishield.yml
}

test-ASSERT-FAIL-13-dashboard_initialize-distributed-two-kibana-nodes-two-fortishield-nodes-curl-error() {
    load-dashboard_initialize
    dashboard_node_names=("kibana1" "kibana2")
    dashboard_node_ips=("1.1.1.1" "1.1.1.2")
    u_pass="user_password"
...
test-14-dashboard_initialize-distributed-two-kibana-nodes-two-fortishield-nodes-curl-error-force() {
    load-dashboard_initialize
    dashboard_node_names=("kibana1" "kibana2")
    dashboard_node_ips=("1.1.1.1" "1.1.1.2")
    u_pass="user_password"
...
test-14-dashboard_initialize-distributed-two-kibana-nodes-two-fortishield-nodes-curl-error-force-assert() {
    installCommon_getPass  admin
    sleep  10
    sleep  10
    sleep  10
