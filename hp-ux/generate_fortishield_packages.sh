...
# Copyright (C) 2018 Fortishield Inc.
# This program is a free software; you can redistribute
# it and/or modify it under the terms of GPLv2
# Fortishield HP-UX Package builder.


install_path="/var/ossec"
current_path=`pwd`
build_tools_path="/home/okkam"
source_directory=${current_path}/fortishield-sources
...
# Needed variables to build Fortishield with custom GCC and cmake
...
build_environment() {
    ...
    # Resizing partitions for Site Ox boxes (used by Fortishield team)
    ...
    curl -k -SO http://fortishield.github.io/packages/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -k -SO http://fortishield.github.io/packages/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    fortishield_version=$(cat ${source_directory}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    echo ${fortishield_version} > /tmp/VERSION
    echo ${fortishield_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/curl -k -L -O https://github.com/fortishield/fortishield/archive/${fortishield_branch}.zip
    /usr/local/bin/unzip ${fortishield_branch}.zip
    mv fortishield-* ${source_directory}
    compute_version_revision
}

check_version() {
    fortishield_version=`cat ${source_directory}/src/VERSION`
...
compile() {
    echo "Compiling code"
    # Compile and install fortishield
    cd ${source_directory}/src
    config
    check_version
    gmake deps RESOURCES_URL=http://fortishield.github.io/packages/deps/${deps_version} TARGET=agent
...
create_package() {
    ...
    fortishield_version=`echo "${fortishield_version}" | cut -d v -f 2`
    pkg_name="fortishield-agent-${fortishield_version}-${fortishield_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_name} ${install_path} /sbin/init.d/fortishield-agent /sbin/rc2.d/S97fortishield-agent /sbin/rc3.d/S97fortishield-agent
...
clean() {
    ...
    find /sbin -name "*fortishield-agent*" -exec rm {} \;
    userdel fortishield
